...
- running `pio run -t uploadfs`, `flash_loop_gui2.py`, Ignition, or
  the JumperIDE "Sync Filesystem" button
...
```bash
cd firmware
python3 flash_loop_gui2.py echo
```

Writes `bootloader.bin` + `firmware.bin` + `fatfs.bin` in a single
`esptool` batch. Add `--post-sync` to also run badge_sync afterwards
(useful when upgrading badges that already had user files).
...
`firmware/scripts/badge_sync.py` is the **single** implementation that
JumperIDE, PlatformIO, raw shells, Ignition, and `flash_loop_gui2`
all reuse. It speaks the MicroPython raw REPL (Ctrl-A) and:
...
flowchart LR
    ...
    FlashLoop["flash_loop_gui2 --post-sync"] --> BadgeSync
...
| `badge_config` | `BadgeConfig` | settings.txt mirror, WiFi credentials |
